...
const UduLogger = function (options) {
  ...
  this.count = async (...args) => {
    const resultCounts = await Promise.all(transports
      .filter(transport => transport instanceof Transports.Elastic)
      .map(transport => transport.count(...args))
    );

    return resultCounts.reduce((acc, cur) => acc + cur, 0);
  }

  this.getUniqueValues = async (...args) => {
    const result = [];
    transports.forEach((transport) => {
      if (transport instanceof Transports.Elastic) {
        result.push(transport.getUniqueValues(...args));
      }
    });
    return Promise.all(result);
  };
  ...
  this.flush = () => {
    return Promise.all( transports.filter( t => typeof t.flush === 'function' ).map( t => t.flush( ) ) );
  };
};
